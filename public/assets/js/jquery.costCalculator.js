	...
	var methods =
	{
		...
		calculate : function(options){
			return this.each(function(){
				...
				$(this).html((options.currencyPosition=="before" ? '<span class="currency">' + options.currency + '</span>' : '')+sum.toFixed(2).replace(".", options.decimalSeparator).replace(regex, '$&' + options.thousandthSeparator)+(options.currencyPosition=="after" ? '<span class="currency">' + options.currency + '</span>' : ''));
